...
def select_bands(arr, names=None):
    ...
    if names:
        names = [str(n).upper().strip() for n in names]
        names = ["B" + n[1:].zfill(2) if n.startswith("B") and n[1:].isdigit() else n for n in names]
...
def to_reflectance(arr, scale=10000.0, offset=0.0):
    ...
    if arr.size and np.nanmax(arr) <= 2.0:   # already reflectance
        return arr
