"""State-space structural backbone used by SYNAPSE Pro.

ADAPTED THIRD-PARTY CODE. The residual-group / VSS-block / SS2D cross-scan design and this implementation are
from MambaSR as distributed in ESAOpenSR/SEN2SR (commit 8e21bb669bcc6e8eb953a1fb24dfc5bb59dc18a4,
sen2sr/models/opensr_baseline/mamba.py), licensed CC0-1.0, which itself builds on VMamba / MambaIR. See
THIRD_PARTY_NOTICES. SYNAPSE changes: the fused mamba_ssm CUDA kernel is replaced by an exact chunked
selective scan (synapse.models.scan) so the backbone runs on any CUDA/CPU host without building mamba_ssm,
and the timm / einops dependencies are replaced by their torch equivalents. Numerics of the recurrence are
unchanged; the SYNAPSE-specific input, frequency, reconstruction, physics and confidence components live in
synapse.models.pro.
"""
import math
...
from synapse_sr.models.scan import selective_scan_fn
...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        if self.p == 0.0 or not self.training:
            return x
        keep = x.new_empty((x.shape[0],) + (1,) * (x.ndim - 1)).bernoulli_(1 - self.p)
